 ...
 * @param {number} size - Size of boxes (default: PLAYER_SIZE)
 * @returns {boolean} True if colliding
 */
function checkAABB(x1, y1, x2, y2, size = CONFIG.PLAYER_SIZE) {
    return (
        x1 < x2 + size &&
        x1 + size > x2 &&
        y1 < y2 + size &&
...
function isInBounds(x, y) {
    return (
        x >= 0 &&
        x <= CONFIG.WORLD_WIDTH - CONFIG.PLAYER_SIZE &&
        y >= 0 &&
        y <= CONFIG.WORLD_HEIGHT - CONFIG.PLAYER_SIZE
    );
}
